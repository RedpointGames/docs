...
:::danger
Do not call `QueryReceipts` from the `OnUnexpectedPurchaseReceipt` event. This will cause an infinite loop, since `QueryReceipts` will invoke the `OnUnexpectedPurchaseReceipt` event.
:::

You _must_ handle deferred purchases on Google Play. Also, you can not know when a deferred purchase fails; this is a limitation of how Google Play Billing works. Instead, you simply don't award items until the receipt moves into a `Purchased` state, so a failed deferred receipt never gets handled by the game.
