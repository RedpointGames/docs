...
2. Refer to the bottom of the [Steamworks Rich Presence](https://partner.steamgames.com/doc/features/enhancedrichpresence) documentation and create the localisation file. Each files underneath `Tokens` should have a key in the format `#<TextNamespace>_<TextKey>` and the value is the translated text value.
