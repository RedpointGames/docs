...
We may remove API versions over time when they are no longer practical to support due to changes in the EOS SDK. Removal of previous API versions is expected to be a slow process (on the time scale of years), so you do not need to immediately prioritize upgrading the API version on existing projects.
...
As of this API version, `externalAccount.epic.displayName` returns the unsanitized display name, and `externalAccount.epic.displayNameSanitized` should be used instead if being displayed to the user.

### 2023-10-27

As of this API version, IsPlayerMuted in voice chat only reports the local mute status of remote players instead of the remote mute status.

### 2022-10-28

As of this API version, the behaviour of `IOnlineAchievements::WriteAchievements` was changed to unlock achievements directly instead of just passing the call through to `IOnlineStats::UpdateStats`.
...
As of this API version, only non-full sessions are returned by default from session results. If you want to include full sessions, add a search filter for `"minslotsavailable" >= 0` (the default is now `"minslotsavailable" >= 1`).
