...
- The [example projects for the Redpoint EOS Online Framework and Online Subsystem Blueprint plugins](./example_project.mdx) have been fully replaced with modern versions.
...
- Fixed the issue that prevented some developers from using the Developer Authentication Tool. You can now authorize accounts for use with the Developer Authentication Tool by clicking "Authorize Epic Games Account" under the "Epic Online Services" toolbar item. Refer to [the authentication documentation](/auth/dev_auth_tool_usage.mdx#authorize-all-the-accounts-you-want-to-use) for more information on how to do this.
...
- Added documentation on how to access the [new Lyra example for Redpoint's EOS Online Framework](example_project.mdx#accessing-the-lyra-example-project). This is a preconfigured version of Lyra set up to use the Redpoint EOS Online Framework plugin instead of Epic's implementation.
...
- Authentication has received a major overhaul in this release:
  - The default login flow now automatically picks the best way to sign in based on what platform or store your game is launched from. If you are updating to this release, you will need to [set your API version to 2021-05-31 or later](/support/advanced_configuration.mdx#2021-05-31) to get this new behaviour.
  ...
  - We added a ["simple first-party" cross-platform account provider](auth/configuration.mdx#using-the-simple-first-party-account-provider) which you can use to do username + password authentication in your game.
...
- Released a new [blueprint-only example project](./example_project.mdx) for EOS Online Framework that uses Online Subsystem Blueprints.
...
- In API version 2020-12-02 and later, the `JoinSessionAccepted` event fired from the EOS SDK is mapped onto `OnSessionUserInviteAccepted` instead of automatically joining the session for you. See [API versions](/support/advanced_configuration.mdx#2020-12-02) for more details.
- In API version 2020-12-09 and later, sessions that are not ready to accept client connections will no longer be returned in search results by default. This means that clients will only find sessions they can actually connect to. If you want to retrieve all sessions or non-listening sessions, refer to [API versions](/support/advanced_configuration.mdx#2020-12-09) on how to do this.
