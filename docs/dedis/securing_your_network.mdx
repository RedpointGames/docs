...
You'll most likely want the same people to have SSH access across all of your dedicated servers. You can set up a policy in CloudFlare Teams, and then use this policy for every application. When you want to update everyone's access, you only need to modify the policy.

1. Under "Access" -> "Policies", click "Add a policy".
...
1. Expand "Browser rendering settings".
1. Enable "Allow automatic Cloudflared authentication".
1. Select "SSH" for "Browser rendering".
1. Under "Access policies", click "Select existing policies".
1. Tick the policy you previously created.
1. Select the identity providers you want to use to authenticate SSH access with at the bottom of the page.
1. Click "Next".
1. Untick "Show application in App Launcher".
1. Click "Next".
1. Click "Save".
