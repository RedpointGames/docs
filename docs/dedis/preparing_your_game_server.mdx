...
If it's set to "Both developers and players will run dedicated servers", you'll also need to pass the client ID and secret as environment variables when defining your `GameServer` and `GameServerFleet` resources, as demonstrated below.
...
spec:
  ...
  containers:
    - name: eos-dedicated-server
      ...
      env:
        - name: REDPOINT_EOS_DEDICATEDSERVERCLIENTID
          valueFrom:
            secretKeyRef:
              name: eos-server-info
              key: DedicatedServerClientId
        - name: REDPOINT_EOS_DEDICATEDSERVERCLIENTSECRET
          valueFrom:
            secretKeyRef:
              name: eos-server-info
              key: DedicatedServerClientSecret
        - name: REDPOINT_EOS_DEDICATEDSERVERPRIVATEKEY
          valueFrom:
            secretKeyRef:
              name: eos-server-info
...
The `:latest` part is the version. If you're automating deployments you might make this the commit hash or source control version for example.
:::
