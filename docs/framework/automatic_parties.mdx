...
You can get the party that the local player is currently in by using the <NodeRef name="Get Primary Party" /> blueprint node on the local player subsystem.
...
You can change the size of the party, if you are the party leader, by using the <NodeRef name="Update Party" /> blueprint node.

<Blueprint height="800px" blueprint={bpUpdatePartySize} />

## Toggle voice chat for automatic parties at runtime

If you have enabled voice chat in automatic parties, you can later turn voice chat on and off for the primary party by using the <NodeRef name="Set Party Voice Chat Enabled" /> blueprint node. You can check if voice chat is enabled for the primary party by using the <NodeRef name="Is Voice Chat Enabled in Primary Party?" pure /> blueprint node.
