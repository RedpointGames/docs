...
The voice chat component simplifies the implementation by allowing you to set the "voice chat group" on each player. Players with the same voice chat group will be able to hear each other. You don't need to know or manage voice channel names when using the voice chat component.
...
- <NodeRef name="Get Audio Anchor Point" /> Returns the actor that this player's
  voice chat audio will be emitted from.
...
- <NodeRef name="Is Input Muted" /> Returns whether this player's microphone is muted
  in this voice chat group.
- <NodeRef name="Set Input Muted" /> Sets whether this player's microphone is muted
  in this voice chat group.
- <NodeRef name="Get Input Volume" /> Returns the volume of this player's microphone
  in this voice chat group.
...
- <NodeRef name="Is Output Muted" /> Returns whether this player's speaker is muted
  in this voice chat group.
- <NodeRef name="Set Output Muted" /> Sets whether this player's speaker is muted
  in this voice chat group.
- <NodeRef name="Get Output Volume" /> Returns the volume of this player's speaker
  in this voice chat group.
...
- <NodeRef name="Get Other Member User Ids" /> Returns the list of other users that
  are currently in this player's voice chat group.
- <NodeRef name="Get All Member User Ids" /> Returns the list of all users (including
  this player) that are currently in this player's voice chat group.
- <NodeRef name="Is Member Output Local Muted" /> Returns if the other member in
  the voice chat group has been locally muted by this player.
- <NodeRef name="Set Member Output Local Muted" /> Sets the other member's audio
  to be locally muted or unmuted by this player.
- <NodeRef name="Get Member Output Local Volume" /> Returns the locally adjusted
...
- <NodeRef name="Is Member Output Hard Muted" /> Returns if the other member in the
  voice chat group has been hard muted by the server.
- <NodeRef name="Is Member Blocked" /> Returns if the other member in the voice chat
  group has been blocked by the local player. If another member of the voice chat
  group is blocked by the local player, the local player's microphone audio will
  not be sent to them.
- <NodeRef name="Is Local User Speaking" /> Returns if the local player is currently
  speaking.
- <NodeRef name="Is Member Speaking" /> Returns if the member of the voice chat group
  is currently speaking.
- <NodeRef name="Get Member Audio Actor" /> Returns the actor that the other member's
  audio is being emitted from. This will be None if the target member is not emitting
  proximity-enabled audio, or if the actor has not been replicated by the server.

The following blueprint nodes can be called only on the server:

- <NodeRef name="Is Hard Muted on Server" /> Returns true if this player has been
...
- <NodeRef name="Get Available Voice Chat Audio Devices" /> Returns the available
...
- <NodeRef name="Set Voice Chat Audio Output Device" /> Sets the output audio device
  that the user should use for voice chat.
- <NodeRef name="Get Voice Chat Audio Input Device" /> Returns the current input
  audio device the user is using for voice chat.
- <NodeRef name="Get Voice Chat Audio Output Device" /> Returns the current output
  audio device the user is using for voice chat.
